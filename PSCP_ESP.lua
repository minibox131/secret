...
local DefaultConfig = {
	...
	FooterText = "Production"
}

-- Config Checker / handler | * Adds new indexes after updates to default config!
...
-- Apply notif mode immediately from config (GUI toggle syncs later in task.defer)
UseCustomNotif = (config.Settings and config.Settings.UseCustomNotif) == true
...
-- ── Notification mode ────────────────────────────────────────
-- Initialised from config once config is loaded; toggled via GUI
local UseCustomNotif = false  -- set after config load below

local function notifyCustom(title, text, time)
	local ok, err = pcall(function()
		local existing = CoreGui:FindFirstChild("PSCP_Toast")
		if existing then existing:Destroy() end

		local toast = Instance.new("ScreenGui")
		toast.Name            = "PSCP_Toast"
		toast.ResetOnSpawn    = false
		toast.DisplayOrder    = 999
		toast.Parent          = CoreGui

		local frame = Instance.new("Frame")
		frame.Size              = UDim2.new(0, 240, 0, 52)
		...
		accent.BackgroundColor3 = Color3.fromRGB(0, 174, 255)
		...
		task.delay(tonumber(time) or 3, function()
			if toast and toast.Parent then toast:Destroy() end
		end)
	end)
	if not ok then warn("[PSCP] notify error:", err) end
end

local function notifyNative(title, text, time)
	pcall(function()
		game:GetService("ReplicatedStorage").NotifGui:Fire(title, text, time or 3)
	end)
	local aud = Instance.new("Sound")
	aud.Parent  = CoreGui
	aud.SoundId = "rbxassetid://" .. (config.NotifSoundId or "9120386436")
	aud.Volume  = 1
	aud:Play()
	game:GetService("Debris"):AddItem(aud, 5)
end

local function notify(title, text, time)
	if UseCustomNotif then
		notifyCustom(title, text, time)
	else
		notifyNative(title, text, time)
	end
end
...
local ActiveItemESP    = {} :: {[Instance]: Folder}
local ModWatchConnection: RBXScriptConnection? = nil
local RoomData         = {}
local ESP_TICK_RATE    = 0.05
...
local ITEM_COLOR   = Color3.fromRGB(255, 220, 60)   -- gold
local ITEM_OUTLINE = Color3.fromRGB(200, 160, 20)

local function addItemHighlight(item: Instance)
	if ActiveItemESP[item] then return end  -- already tracked

	local holder = Instance.new("Folder")
	holder.Name   = item.Name .. "_ITEM_ESP"
	holder.Parent = CoreGui
	ActiveItemESP[item] = holder

	local highlight = Instance.new("Highlight")
	highlight.Adornee      = item
	highlight.FillColor    = ITEM_COLOR
	highlight.OutlineColor = ITEM_OUTLINE
	highlight.FillTransparency    = 0.4
	highlight.OutlineTransparency = 0
	highlight.Parent       = holder

	-- Billboard label — adorn to the item's PrimaryPart or first BasePart
	local adornPart = item:IsA("BasePart") and item
	...
	-- Clean up if the item is removed from the world
	local removeConn
	removeConn = item.AncestryChanged:Connect(function(_, parent)
		if not parent then
			if holder.Parent then holder:Destroy() end
			ActiveItemESP[item] = nil
			removeConn:Disconnect()
		end
	end)
end

local function removeItemHighlight(item: Instance)
	local holder = ActiveItemESP[item]
	if holder then
		if holder.Parent then holder:Destroy() end
		ActiveItemESP[item] = nil
	end
...
closeBtn.MouseButton1Click:Connect(function()
	...
	config.Settings.UseCustomNotif= UseCustomNotif
	SaveConfig()
	screenGui:Destroy()
end)
...
local function toggleButton(label: string, order: number, onToggle: (state: boolean) -> ())
	...
	local function setToggle(newState: boolean)
		state = newState
		onToggle(state)
		if state then
...
notifToggle.Size             = UDim2.new(1, 0, 0, 30)
notifToggle.BackgroundColor3 = Color3.fromRGB(22, 22, 28)
notifToggle.BorderSizePixel  = 0
notifToggle.TextColor3       = Color3.fromRGB(160, 160, 175)
notifToggle.TextSize         = 12
notifToggle.Font             = Enum.Font.Gotham
notifToggle.Text             = "Custom notifications  ●  OFF"
notifToggle.TextXAlignment   = Enum.TextXAlignment.Left
notifToggle.LayoutOrder      = 3
...
notifPad.Parent      = notifToggle

notifToggle.MouseButton1Click:Connect(function()
	UseCustomNotif = not UseCustomNotif
	config.Settings.UseCustomNotif = UseCustomNotif
	SaveConfig()
	if UseCustomNotif then
		notifToggle.Text      = "Custom notifications  ●  ON"
		notifToggle.TextColor3 = Color3.fromRGB(80, 200, 120)
	else
		notifToggle.Text      = "Custom notifications  ●  OFF"
		notifToggle.TextColor3 = Color3.fromRGB(160, 160, 175)
	end
	notify("Notifications", UseCustomNotif and "Using custom toasts" or "Using game notifications")
end)

-- ── Visual section ───────────────────────────────────────────

sectionLabel("Visual", 4)

local _, setESP = toggleButton("Class ESP", 5, function(state)
...
local _, setChams = toggleButton("Class Chams", 6, function(state)
...
sectionLabel("Item ESP", 7)

inputRow("Item name…", 8, function(itemName)
	local itemsFolder = workspace:FindFirstChild("Round")
		and workspace.Round:FindFirstChild("Items")
	if not itemsFolder then
		notify("Item ESP", "workspace.Round.Items not found")
		return
	end

	local item = itemsFolder:FindFirstChild(itemName)
	if not item then
		notify("Item ESP", "\"" .. itemName .. "\" not found")
		return
	end

	if ActiveItemESP[item] then
		removeItemHighlight(item)
		notify("Item ESP", itemName .. " highlight OFF")
	else
		addItemHighlight(item)
		notify("Item ESP", itemName .. " highlight ON")
	end
end)

-- ── Room ESP ─────────────────────────────────────────────────

sectionLabel("Room ESP", 9)

inputRow("Room name…", 10, function(roomName)
	RoomESP(roomName)
end)

-- ── Server section ───────────────────────────────────────────

sectionLabel("Server", 10)

local _, setModWatch = toggleButton("Mod Watch", 11, function(state)
...
task.defer(function()
	...
	-- Notif mode — apply silently before anything else fires notify()
	if s.UseCustomNotif then
		UseCustomNotif = true
		notifToggle.Text       = "Custom notifications  ●  ON"
		notifToggle.TextColor3 = Color3.fromRGB(80, 200, 120)
	end
